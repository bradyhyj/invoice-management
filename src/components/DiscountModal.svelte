<script>
  ...
  $: discountableAmount = validItems.reduce((sum, item) => sum + Number(item.amount), 0); // 할인 적용 가능한 품목 합계
  ...
  $: calculatedDiscount = (() => {
    ...
    if(policy.type === 'POINT_INPUT') { // 포인트를 직접 입력하는 경우
    ...
    else if (policy.type === 'PERCENT_TIER' || policy.type === 'PERCENT_OFF') {
      // 선택된 품목들의 합계 계산
      const selectedTotal = validItems
        .filter(item => selectedIndices.includes(item.originalIndex))
        .reduce((sum, item) => sum + Number(item.amount), 0);
      
      let rate = policy.type === 'PERCENT_TIER' ? policy.tiers[selectedTier] : policy.rate;
      ...
      // 최대 한도 적용
      let limit = policy.type === 'PERCENT_TIER' ? policy.maxLimits[selectedTier] : policy.maxLimit;
  ...
  function handleApply() {
    ...
    if (policy.type === 'POINT_INPUT') {
      ...
      if (policy.minUsage && calculatedDiscount < policy.minUsage) {
        return alert(`포인트는 최소 ${policy.minUsage.toLocaleString()}P 이상부터 사용 가능합니다.`);
      }
...
<div class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
  ...
  <div class="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
    ...
    <div class="p-6 overflow-y-auto space-y-6">
      ...
      <!-- 안내 문구 (노란색 박스) -->
      <div class="bg-yellow-50 border border-yellow-200 p-3 rounded text-sm text-yellow-800 whitespace-pre-line leading-relaxed">
        ...
        <div class="space-y-3">
          <div class="flex gap-4">
            <label class="flex items-center gap-2 cursor-pointer">
              <input type="radio" bind:group={customType} value="amount" class="w-4 h-4 text-indigo-600" />
              <span class="font-medium text-slate-700">금액(원) 할인</span>
            </label>
            <label class="flex items-center gap-2 cursor-pointer">
              <input type="radio" bind:group={customType} value="percent" class="w-4 h-4 text-indigo-600" />
          ...
          <div class="flex items-center gap-2">
            <input 
              ...
              placeholder={customType === 'amount' ? "할인할 금액 입력" : "할인율(%) 입력"} 
              class="flex-1 border border-slate-300 rounded-lg px-4 py-2 outline-none focus:border-indigo-500"
            />
            <span class="font-bold text-slate-600">{customType === 'amount' ? '원' : '%'}</span>
          </div>
        </div>
      {/if}
        ...
        <div class="space-y-2">
          ...
          <div class="flex gap-4">
            {#each Object.keys(policy.tiers) as tier}
              <label class="flex items-center gap-2 cursor-pointer">
                <input type="radio" bind:group={selectedTier} value={tier} class="w-4 h-4 text-indigo-600" />
                <span class="font-medium text-slate-700">{tier} ({(policy.tiers[tier]*100)}%)</span>
              </label>
            {/each}
          </div>
        </div>
      ...
      {#if policy.requiresItemSelection}
        <div class="border border-slate-200 rounded-lg overflow-hidden">
          <div class="bg-slate-100 px-4 py-2 font-bold text-sm text-slate-700 flex justify-between items-center">
            <span>할인 적용할 품목 선택</span>
            <label class="flex items-center gap-2 cursor-pointer text-xs font-normal">
              <input type="checkbox" on:change={toggleAllSelection} class="rounded text-indigo-600" />
              전체선택
            </label>
          </div>
          <div class="max-h-40 overflow-y-auto divide-y divide-slate-100">
            {#if validItems.length === 0}
              <div class="p-4 text-center text-sm text-slate-500">할인을 적용할 수 있는 품목이 없습니다.</div>
            {/if}
            {#each validItems as item}
              <label class="flex items-center justify-between p-3 hover:bg-slate-50 cursor-pointer">
                <div class="flex items-center gap-3">
                  <input type="checkbox" bind:group={selectedIndices} value={item.originalIndex} class="w-4 h-4 rounded text-indigo-600" />
                  <span class="text-sm font-medium text-slate-700">{item.name} <span class="text-slate-400 text-xs">({item.qty}개)</span></span>
                </div>
                <span class="text-sm font-bold text-slate-900">{Number(item.amount).toLocaleString()}원</span>
              </label>
            {/each}
          </div>
        </div>
        ...
        <div class="space-y-4">
            ...
            <div class="p-4 space-y-4">
                ...
                <div class="flex justify-between items-end mb-2">
                    <label class="text-sm font-bold text-slate-700">사용할 포인트 입력</label>
                    <button 
                    class="text-xs font-medium text-slate-500 hover:text-slate-800 underline decoration-slate-300 underline-offset-2"
                    on:click={() => {
                        // 할인 최대 적용 가능한 금액을 계산
                        customValue = getMaxPointLimit(discountableAmount, policy);; 
                    }}
                    >
                    할인 최대 적용
                    </button>
                ...
                <p class="text-[11px] text-slate-400 mt-2">* 1P는 1원으로 환산되어 결제 금액에서 차감됩니다.</p>
                </div>
            </div>
            </div>
        </div>
    ...
    <!-- 푸터 (결과 표시 및 버튼) -->
    <div class="border-t border-slate-200 p-4 bg-slate-50 flex items-center justify-between rounded-b-xl">
      <div>
        <div class="text-xs text-slate-500 font-bold mb-1">할인금액</div>
        <div class="text-2xl font-black text-red-600">-{calculatedDiscount.toLocaleString()}원</div>
      </div>
      <div class="flex gap-2">
        <button on:click={onClose} class="px-5 py-2.5 rounded-lg font-bold text-slate-600 bg-white border border-slate-300 hover:bg-slate-50">취소</button>
        <button on:click={handleApply} class="px-5 py-2.5 rounded-lg font-bold text-white {policy.buttonColor.split(' ')[0]} hover:brightness-95">할인 적용하기</button>
      </div>
    </div>
  </div>
</div>